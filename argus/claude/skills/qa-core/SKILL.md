...
- Compute each expected value independently from the specification, never from the live response it judges. A snapshot of current application behaviour is a change-detector, not a correctness oracle; never pin an expectation to it. When an exact formula is unknown or too costly, assert a metamorphic invariant rather than invent a point value.
