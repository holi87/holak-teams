...
- New page objects in Atlas's shared layer (extend, never fork) for screens she did not yet model, including the `ctaEnablement(form)` helper that drives `{empty, each required field missing, one invalid value, all valid}` against every mutating control and returns the enabled/disabled outcome per cell, so Penelope's enablement baseline is encoded GREEN once and reused.
