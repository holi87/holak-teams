...
- **Generated conformance grid (mandatory output).** Emit `solution/paths/conformance-grid.json` as `{"$schema": "argus/conformance-grid@1", "schemaVersion": 1, "engagementId", "rows": [], "residuals": []}`: one row for every operation × documented **exact** response code, and one row for every request field × schema constraint (`type`, `format`, `enum`, `minimum`/`maximum`, `minLength`/`maxLength`, `pattern`, `required`), each carrying the contract-derived expected outcome and its source citation. Rows are data for Talos, never prose. An operation, a constrained field, or a response documented only as a wildcard or catch-all — which has no exact status to assert — is a named residual, not an omission.
