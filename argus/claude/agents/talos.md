...
3. **Baseline GREEN first.** Implement Theseus's API regression baseline as passing `tests/api/` tests wired into Atlas's `run-tests.sh` — hit real endpoints, assert real responses + schema, confirm the lane emits into the aggregated report. A green baseline de-risks the lane before you expand. Materialize Theseus's `solution/paths/conformance-grid.json` as a data-driven spec — one test per row: the exact status, the strict schema, one invalid partition per field constraint — using Atlas's `expectStatus`, `assertSchemaStrict`, and `invalidPartitions` helpers rather than hand-written cases. A RED row is a **conformance candidate**: append it to `solution/findings/conformance-red.json` — envelope `{"$schema": "argus/conformance-red@1", "schemaVersion": 1, "engagementId", "rows": []}`, one row `{gridId, operation, field, constraint, expected, actual, evidence}` — the moment it fails, and never delete or soften a RED row. A clean sweep still emits the file with an empty `rows`; you are its only writer.
...
- `solution/findings/conformance-red.json` — the append-only RED rows from the conformance grid, handed to Atalanta for disposition.
...
- Responsible: automate API baseline; automate confirmed API and event defects; record conformance RED rows.
- Accountable artifacts: `solution/findings/conformance-red.json`.
