...
- **(e) Scope of every cardinality rule — vary the actor, not only the count (decision table + state-transition).** For each documented `once` / `at most N` / `only the assigned X` rule, read its scope noun first (per entity, per user, per actor, per day). Then hold that scope fixed and vary every OTHER dimension: a second actor in the same role (after a legitimate reassignment, or via the claim-hijack path when that defect exists), a second entity, a later time window. The limit must bind on the documented scope and only there — a `once per entity` rule that resets when the actor changes, or a `once per actor` rule that leaks across entities, IS the defect. A rule exercised with one actor only is un-covered, not clean. Record the scope-varied dimensions in the STATE_MODEL transition row.
- **(f) Post-change re-evaluation of in-flight entities (state-transition + decision table).** After a rule or config change — price table, threshold, product parameter, quota — assert three separate oracles and file each violation on its own: **(i) decided or terminal entities are frozen**, their recorded terms unchanged; **(ii) in-flight entities are re-evaluated or flagged**, so an entity that no longer meets the new criteria is visibly marked; **(iii) transition guards re-check current criteria**, so the commit transition (approve, publish, pay) is blocked or warned for an entity that now fails the rule. Proving (i) does not cover (ii) or (iii); "live recompute of eligibility is out of scope" is never a valid narrowing of this class.
