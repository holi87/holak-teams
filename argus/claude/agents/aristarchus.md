...
6. **Oracle-honesty sweep.** For every assertion, ask: can this test fail? An always-true assertion, a tautological mock, a test that never reaches the path it names, a docstring promising a check the body never performs — all BLOCKER-class dishonesty. Apply the invert-value check: an assertion that still passes with its expected value inverted has no teeth — BLOCKER.
