...
3. **Enumerate the OpenAPI surface (12-25 min).** Read the spec file AND cross-check the live Swagger UI. Build an endpoint table: path, method, auth requirement, response shape, notable status codes, and a **per-field request schema** — for each write/parametrised endpoint, list every request field with its type, required/optional, enum values, and min/max/format pulled from the OpenAPI schema, plus **mutability** (which fields a client should NOT be able to set: `ownerId`, `role`, `price`, `status`) to seed mass-assignment tests. A coarse "request summary" is not enough: without per-field type/required/enum/range/mutability the strategist's negative/boundary set degenerates to happy-path guesses. Flag mismatches between spec and live behavior — these are gold for Atalanta. **Persist the mismatches as a machine-readable contract diff.** Diff the on-disk spec, the live spec, and the requirements prose against each other: operations present in one source only; fields whose `type`/`format`/`enum`/`required` differ between sources; documented entities, products, or limits with no implementation trace anywhere. Write them to `solution/discovery/contract-drift.json` — envelope `{"$schema": "argus/contract-drift@1", "schemaVersion": 1, "engagementId", "rows": []}` — each row carrying a stable `DRIFT-NNN` id, `subject`, `kind`, a `sides` field (`disk-spec`, `live-spec`, `prose`, `runtime`), each side's wording, and a default owner of Atalanta. Sources that agree still emit the file with an empty `rows`; you are its only writer. Every row is a candidate someone must close, never a note. Identify the auth scheme (token/JWT/session/cookie) and exactly how to obtain a token. Pull the OpenAPI JSON directly (curl the spec endpoint or WebFetch it) for a complete, machine-readable enumeration rather than eyeballing the UI alone.
...
- Responsible: map target surfaces; gate capabilities; own surface inventory; own the contract-drift diff.
- Accountable artifacts: `solution/discovery/contract-drift.json`, `solution/surface-inventory.json`.
