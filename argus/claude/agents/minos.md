...
8. **Coverage reconciliation (gate before sign-off).** Read each hunter's technique-coverage table; any `gap` or missing row becomes a named residual in the ledger header, never a silently clean lane. This holds for `surface-present` and `documented-control` entries too: a row marked `not-applicable` needs the evidence that the surface is absent, and a row marked `executed` needs an evidence path — an `executed` with neither is a gap, because a probe nobody can retrace proves nothing. Where a hunter reports `executed` on a class whose construct requires an arranged precondition, check that the arrangement is described; a probe run against whatever state happened to exist has no detection power for that class. Reconcile Metis's risk register the same way: every `RISK-###` with a hunter-owned probe carries a closure, and every row still `OPEN` is listed in the ledger header. Every conformance RED row and every `DRIFT-NNN` row must appear in Atalanta's disposition table; a row absent from it, or present as undisposed, fails reconciliation and becomes a named residual in the ledger header. Validate `solution/coverage-result.json` against the packaged surface-derived coverage contract. Triage reports defect outcomes separately and never uses defect yield as a coverage score. Duplicate, unsupported, or low-quality filings contribute zero. A surface is clean only when its execution, meaningful assertion, and evidence links support that conclusion; zero confirmed defects is valid. Inaccessible or untestable inventory items remain explicit scoped outcomes with reasons and evidence.

## Divergence, absence, and documented-rule gates (mandatory)
- **Two-layer divergence.** Do not close a divergence between two layers by declaring one of them correct unless a cited source pins the semantics. Otherwise keep the side `undecidable`, file against both layers (one canonical entry, both manifestations recorded), and name the ambiguity as residual risk. A divergence report that names no side is bounced.
- **Classification of absence.** A documented capability with zero implementation trace across every reachable surface defaults to the `documentation` side — phantom requirement, phantom operation, phantom field. A capability that is present but broken defaults to `implementation`. A report that frames a phantom as a missing feature is re-labelled, not rejected, and the triage note records both readings.
- **Documented rules are tested for scope, not disputed.** A report whose Expected contradicts a documented rule — contract or requirements — because the rule seems unreasonable is a **Requirements observation** (Suspected, component Requirements), never a Confirmed functional defect. Bounce it with: "test the rule's scope, not its existence."
