...
The runtime-asset sync generates a byte-identical installed reference at
`${CLAUDE_PLUGIN_ROOT}/references/SHARED-DOCTRINE.md`. Its current purpose is anti-drift
and runtime inspection; behavioral blocks remain inline until the preloaded-skill work
tracked separately is validated.
...
## Decision — why behavioral rules still stay inline

Packaging this reference does not itself externalize behavioral/hardening blocks. A
fire-and-forget worker can still skip an ad-hoc Read or run out of budget. Until a
preloaded skill guarantees delivery and its regression evaluation passes, losing a
hardening rule remains a silent coverage risk.
...
- **This file is the anti-drift canon** — edit shared wording here, then run the runtime
  asset sync and mirror intentional inline wording.
...
Externalization remains gated behind a mechanism that **guarantees** the shared content
reaches the subagent (for example, a preloaded skill or an install-time inliner) — not a
runtime "please read" reference.
