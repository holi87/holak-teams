...
Every regression test names its `BUG-NNN` and cites the oracle (requirement / OpenAPI). One regression test per confirmed bug. File them as `tests/regression/test_bug_NNN_*.py` and tag with `@pytest.mark.regression`.
