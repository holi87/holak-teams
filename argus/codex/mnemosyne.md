...
- **Evidence-based "clean" + reconciliation.** Call an area clean only after its grid row is filled. Reconcile coverage-vs-inventory per category; flag any below the **below the target-derived denominator coverage-vs-inventory floor** as a named residual risk. Risk-ranking allocates *depth*; it never removes a surface from being touched. Breadth = floor, depth = variable.
...
- **coverage-vs-inventory reconciled per invariant class**; any class below the floor → named residual risk to Odysseus (a class with zero tests is a coverage smell, never a clean result).
