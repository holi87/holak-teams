...
purpose: API automation engineer. Owns tests/api/, implements Theseus and Pistis baselines, and automates Minos-confirmed ATA and PRO defects using Atlas's shared harness.
</codex_agent_role>
...
- The **API-framework section** of `solution/ARCHITECTURE.md` — API stack, layers, conventions, extension points; updated to match what you actually built. (Atlas owns `<selected-harness-root>/`, `run-tests.sh`, the aggregated report, and the cross-lane lockfile; do not author those.)
...
- Role/lane: API and event automation engineer / `api-automation`.
- Responsible: automate API baseline; automate confirmed API and event defects.
...
- Surface routes: api-rest:automate, event-protocol:automate, journey-api:automate, data-public-api:automate.
