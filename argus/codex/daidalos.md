...
2. **Verify the current Playwright API.** Before writing a line, call context7: `resolve-library-id` then `query-docs` for Playwright (and `@axe-core/playwright`) — config keys, locator APIs, `expect(page)` assertions, reporter flags, and the axe integration drift. Do NOT code from stale memory. If context7 is unavailable, WebFetch the official docs. Confirm reporters are Playwright-native (`list` + `html` + `json`); **no JUnit** (build path — in Mode D / adapt path the repo's EXISTING runner/reporting wins, per the Adopt-or-Build gate). Honour Atlas's reporter wiring — your suite emits into the ONE aggregated report, never a private second report.
