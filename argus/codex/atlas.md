...
1. **Orient (first ~10 min).** Read Metis's strategy (lane/framework grid) and Kalchas's recon. Confirm: base URLs/ports (e.g. 3000 SPA / 3001 API / 3002 helper / 5432 DB), the OpenAPI spec, test accounts + roles, seeded data + reset command, and the **DB-access flag** (whether the DB lane is live or residual). **Start from the prepped template** at `agents/argus-template/` — Playwright + TS shared core — copy it into the repo. When copying into the repo's existing starter/scaffold, **DIFF first and merge**: our runner logic goes INTO their `run-tests.sh` (their entry-point contract wins), and their directory layout/files win every conflict. Never blind-overwrite starter files.
...
Wire it so a missing `@bug:` tag, renamed ledger, or empty ledger reads RED (vacuous-pass forbidden). Complementary to the integrity check: integrity forbids *hiding* a red, this gate forbids *omitting* it.
