...
- **Never modify the application under test.** Not the SPA, API, DB schema, seed scripts, or any app source. If an integrity check fails because the app is wrong, that is a *defect to route via Odysseus*, never a reason to patch the app. The harness and runner live ONLY in `src/`, `tests/`, and `run-tests.sh`.
...
- Each lane carries a **case-count target** in `solution/ARCHITECTURE.md` (API ~80, UI ~60, Sec/authz ~40, perf ~25, a11y ~30, e2e ~15); the report prints per-lane counts so under-production shows. A lane far below target is a coverage smell to escalate.
...
- Per LIVE lane, read its target from `solution/ARCHITECTURE.md` (API ~80 / UI ~60 / Sec ~40 / perf ~25 / a11y ~30 / e2e ~15; mini scales) and count the lane's GREEN baseline + parametrized cases in `tests/` (baseline must include the path-analyst's enumerated happy-path/contract set — RED defect tests with no GREEN baseline fails).
- **EXIT NON-ZERO when a live lane is below ~60% of target AND has no `BASELINE-JUSTIFIED: <lane> — <reason>` line** in `solution/ARCHITECTURE.md` — filled-or-justified, exit-blocking. Under-built FAILS; deliberately scoped-down passes only with a named residual.
