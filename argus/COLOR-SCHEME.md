...
Claude Code officially supports only **8 NAMED colours**: `red, blue, green, yellow, purple, orange, pink, cyan` (hex values are silently ignored).
...
| **purple** | Cross-cutting / meta | architecture, code review, white-box analysis (inform every lane) |
...
**purple — cross-cutting / meta (4):** Atlas (Automation Architect) · Aristarchus (Code Reviewer, runs LAST) · Tiresias (White-box Source Analyst — gated on source-code access) · Asklepios (Test-Suite Sanitation / deflaking — brownfield Mode D)
...
- If you ever want the "additional vs original" cohort distinction back in colour, the alternative
  is: core+rerole = `cyan`, whole D-team = one colour (e.g. `purple`). We chose role-type instead.
