...
8. **Coverage reconciliation (gate before sign-off).** Read each hunter's technique-coverage table; any `gap` or missing row on an `appliesWhen: always` entry becomes a named residual in the ledger header, never a silently clean lane. Reconcile Metis's risk register the same way: every `RISK-###` with a hunter-owned probe carries a closure, and every row still `OPEN` is listed in the ledger header. Every conformance RED row and every `DRIFT-NNN` row must appear in Atalanta's disposition table; a row absent from it, or present as undisposed, fails reconciliation and becomes a named residual in the ledger header. Validate `solution/coverage-result.json` against the packaged surface-derived coverage contract. Triage reports defect outcomes separately and never uses defect yield as a coverage score. Duplicate, unsupported, or low-quality filings contribute zero. A surface is clean only when its execution, meaningful assertion, and evidence links support that conclusion; zero confirmed defects is valid. Inaccessible or untestable inventory items remain explicit scoped outcomes with reasons and evidence.
