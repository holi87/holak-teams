...
grep -Fq $'preflight-report\thttps://raw.githubusercontent.com/holi87/holak-teams/master/argus/schemas/preflight-report.schema.json\tschemaVersion=2\treadCompatible=1,2\treport-only' <<<"$schema_listing" || fail 'schema list omitted the versioned report-only preflight reader'
...
for kind in lane-plan evidence-reference automation-status; do
  "$CLI" schema validate --kind "$kind" --input "$FIXTURES/legacy-valid/$kind-v1.json" >/dev/null
  if "$CLI" schema validate --kind "$kind" --input "$FIXTURES/legacy-invalid/$kind-v1.json" >/dev/null 2>&1; then
    fail "$kind runtime reader accepted an invalid legacy v1 fragment"
  fi
done
...
jq '.engagementId = "schema-fixture"' "$FIXTURES/legacy-valid/lane-plan-v1.json" >"$WORK/lane-kalchas.json"
...
jq -e '."$schema" == "argus/lane-plan@2" and .schemaVersion == 2 and (.lanes[0].transitions | map(.to) == ["planned", "running", "completed"]) and .lanes[0].transitions[1].at == "2026-07-10T00:00:30.000Z"' "$TARGET/$(jq -r .path <<<"$lane_fragment")" >/dev/null || fail 'historical lane-plan v1 fragment was not deterministically normalized and persisted as v2'
...
jq '.engagementId = "schema-fixture"' "$FIXTURES/legacy-valid/evidence-reference-v1.json" >"$WORK/evidence-1.json"
...
jq -e '."$schema" == "argus/evidence-reference@2" and .schemaVersion == 2' "$TARGET/$(jq -r .path <<<"$evidence_fragment")" >/dev/null || fail 'evidence-reference v1 fragment was not persisted as v2'
...
jq '.engagementId = "schema-fixture"' "$FIXTURES/legacy-valid/automation-status-v1.json" >"$WORK/automation-1.json"
...
jq -e '."$schema" == "argus/automation-status@2" and .schemaVersion == 2' "$TARGET/$(jq -r .path <<<"$automation_fragment")" >/dev/null || fail 'automation-status v1 fragment was not persisted as v2'
...
printf 'PASS  Argus schemas: fixtures, v1/v2 lane transitions, v1-to-v2 migration, deterministic collection merges, fragment rejection, stable IDs, runner results, and source-versioned summary\n'
