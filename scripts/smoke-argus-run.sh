...
require_text 'argus-assets model route --manifest' "$SKILL" "run skill does not use capability-bound model routing"
require_text '--dispatch-id <dispatch-id> --attempt 1' "$SKILL" "run skill does not bind the first model attempt"
require_text 'schema validate --kind model-escalation-request' "$SKILL" "run skill does not validate worker escalation envelopes"
require_text 'Start a new agent thread' "$SKILL" "run skill can resume a thread under a different model"
require_text 'argus-assets template detect' "$SKILL" "run skill does not detect template capabilities"
require_text 'template select' "$SKILL" "run skill does not require explicit template selection"
require_text 'TEMPLATE-CONTRACT.md' "$SKILL" "run skill does not load the packaged template contract"
