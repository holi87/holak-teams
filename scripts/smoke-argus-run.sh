...
require_text 'argus-assets orchestration plan --mode <A|B|C|D> --output ai_agents_internal/orchestration-plan.json' "$SKILL" "run skill does not persist the machine orchestration projection"
...
require_controller_text 'Allocate one controller lease and one per worker.' "controller does not allocate isolated worker leases"
